...
public class CredentialStoreUpdatesTestCase extends AbstractSubsystemTest {
    ...
    private static final Logger LOG = Logger.getLogger(CredentialStoreUpdatesTestCase.class.getSimpleName());
    ...
    // 1 prazdny keystore, novy alias + password
    // create credential store (before test) /subsystem=elytron/credential-store=myCredStore:add(location=mynewcredstore.cs, relative-to=jboss.server.config.dir, create=true, credential-reference={clear-text=StorePassword})
    // create key store /subsystem=elytron/key-store=test-keystore:add(path=myKeystore.keystore, type=JKS, credential-reference={store=myCredStore, alias=newAlias, clear-text=newPassword})
    // read aliases /subsystem=elytron/credential-store=myCredStore:read-aliases()
    // read clear-text /subsystem=elytron/credential-store=myCredStore:read-attribute(name=credential-reference.clear-text)
    // read clear-text from keystore (here it is removed?) /subsystem=elytron/key-store=test-keystore:read-attribute(name=credential-reference.clear-text)
    // aj alias sa tu da precitat /subsystem=elytron/key-store=test-keystore:read-attribute(name=credential-reference.alias) - ale v cred store sa nedal, bol undefined
    // aj cred store: /subsystem=elytron/key-store=test-keystore:read-attribute(name=credential-reference.store)

    // vypise vsetko o credential store: /subsystem=elytron/credential-store=myCredStore:read-resource()
    // vypise vsetko o key store: /subsystem=elytron/key-store=test-keystore:read-resource()
    @Test
    ...
    // 2 prazdny keystore, bez aliasu (vygeneruje sa nahodny), password
    @Test
    ...
    // 3 keystore neni prazdny, novy alias a password
    @Test
    ...
    // 4 keystore neni prazdny, bez aliasu - vygeneruje sa, password
    @Test
    ...
    // 5 keystore neni prazdny, pouzije sa alias co uz je v keystore, prepise sa = ulozi sa do neho nove heslo
    @Test
    ...
    // 6 to iste ako testCredentialReferenceAddNewEntry ale v tomto pripade sa ma credential store automaticky vytvorit pocas inej operacie
    @Test
...
// why not the following? // TODO
    ...
    // 7 to iste ako testCredentialReferenceAddNewEntryWithGeneratedAlias ale v tomto pripade sa ma credential store automaticky vytvorit pocas inej operacie
    @Test
    ...
    // 8 to iste ako testCredentialReferenceUpdateExistingEntry ale v tomto pripade sa ma credential store automaticky updejnut pocas inej operacie
    @Test
    ...
    // 9 podobne ako predosly test ale tu sa updejtuje konkretne clear-text atribut
    // posledna ukazka v https://developer.jboss.org/people/fjuma/blog/2019/09/05/automatic-updates-of-credential-stores
    @Test
    ...
    // 10 nezadane heslo takze nic sa nestane, neprazdny keystore
    @Test
    ...
    // 11 len si ulozim novy atribut clear-text, bude ulozeny v managment modeli
    @Test
    ...
    // 12 rollback, adding NEW keystore with invalid type
    @Test
    ...
    // 13 rollback, updating EXISTING keystore with invalid type
    @Test
    ...
    // 14 fail clear-text atribut only
    @Test
    ...
    // 15 rollback of newly added credential, fail during runtime operation
    @Test
    ...
    // 16 rollback of updating existing credential, fail during runtime operation
    @Test
    ...
    // 17 fail clear-text attribute only, during runtime operation
    @Test
...
//        List<String> lines = Arrays.asList(operation.toString(), "===============");
